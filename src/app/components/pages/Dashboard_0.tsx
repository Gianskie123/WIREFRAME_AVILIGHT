...
// ── Site Detail Panel (shown when a site is clicked) ─────────────────────────
function SiteDetailPanel({
  site, selectedYear, selectedMonth, activeEnvFilter, landTempMode,
  lightMode, card, subText, headingText, onClose,
}: {
  site: ObsSite; selectedYear: number; selectedMonth: number;
  activeEnvFilter: EnvFilter|null; landTempMode: LandTempMode;
  lightMode: boolean; card: string; subText: string; headingText: string;
  onClose: () => void;
}) {
  ...
  // Get env value for this site's city
  const city = MM_CITIES.find(c => c.name === cityName);
  const yearIdx = selectedYear - 2014;
  const monthFactor = selectedMonth === 0 ? 1.0
    : [1.02,1.04,1.03,0.98,0.97,0.96,0.95,0.96,0.98,1.00,1.01,1.03][selectedMonth-1];

  function getEnvValue(): string {
    if (!activeEnvFilter || !city) return "—";
    if (activeEnvFilter === "landcover") {
      const dominant = city.dominantLandCover;
      const pct = city.landCoverBreakdown[dominant] ?? 0;
      return `${dominant} (${pct}%)`;
    }
    if (activeEnvFilter === "ndvi") return (city.ndvi * monthFactor * (1 - yearIdx*0.003)).toFixed(3);
    if (activeEnvFilter === "viirs") {
      const yf = 1 + yearIdx * 0.008;
      return `${(city.viirs * yf * (selectedYear===2020?0.92:1)).toFixed(1)} nW`;
    }
    if (activeEnvFilter === "landtemp") {
      const base = landTempMode==="Day" ? city.landTempDay : city.landTempNight;
      const adj = base + yearIdx*0.05 + (selectedMonth===0?0:[0.2,0.4,0.6,0.7,0.5,0.3,0.1,-0.1,-0.2,0.0,0.1,0.2][selectedMonth-1]);
      return `${adj.toFixed(1)}°C`;
    }
    // precip
    const base = city.precip * (selectedMonth===0?1:[0.4,0.3,0.3,0.5,0.8,1.4,1.7,1.6,1.5,1.1,0.8,0.5][selectedMonth-1]);
    return `${Math.round(base)} mm`;
  }

  const ENV_LABELS: Record<EnvFilter,string> = {
    landcover:"Land Cover", ndvi:"NDVI", viirs:"VIIRS (Light Intensity)", landtemp:`Land Temp (${landTempMode})`, precip:"Precipitation",
  };
    ...
    <div className={`${card} p-4 space-y-3`}>
      ...
      {activeEnvFilter && (
        <div className={`rounded-lg p-3 ${lightMode?"bg-gray-50 border border-gray-200":"bg-[#0f172a] border border-[#2a2f42]"}`}>
          ...
          <p className={`text-base font-bold ${headingText}`}>{getEnvValue()}</p>
        </div>
      )}
...
function MetroManilaMap({
  lightMode, selectedYear, selectedMonth, showBirds, activeEnvFilter, landTempMode, selectedLandTypes,
  onSiteSelect, selectedSiteIdx,
}:{
  ...
  selectedLandTypes:Set<LandType>;
  onSiteSelect: (idx: number|null) => void;
  selectedSiteIdx: number|null;
}) {
  ...
  function cityFill(city:MMCity):string {
    if(activeEnvFilter==="landcover"){
      const checked = selectedLandTypes.has(city.dominantLandCover);
      return checked
        ? LAND_COLORS[city.dominantLandCover]+"dd"
        : LAND_COLORS[city.dominantLandCover]+"28";
    }
        ...
        <svg viewBox={`0 0 ${MM_W} ${MM_H}`} preserveAspectRatio="xMidYMid meet" className="w-full h-full"
          ...
          {showBirds && (() => {
            function renderSite(site: ObsSite, i: number, isHov: boolean) {
              ...
              // Hover tooltip: show only species counts per category
            ...
            return (
              <>
                {OBS_SITES.map((site,i)=> hovObs!==i ? renderSite(site,i,false) : null)}
...
function HistoricalPanel({lightMode,selectedYear,selectedMonth,activeEnvFilter,landTempMode,selectedLandTypes,card,subText,headingText}:{
  ...
  const monthFactor = selectedMonth === 0 ? 1.0
    : [1.02,1.04,1.03,0.98,0.97,0.96,0.95,0.96,0.98,1.00,1.01,1.03][selectedMonth-1];
  const yearFactor = 1 + yearIdx * 0.008;

  const avgNdvi  = (MM_CITIES.reduce((s,c)=>s+c.ndvi,0)/MM_CITIES.length) * monthFactor * (1 - yearIdx*0.003);
  const avgViirs = (MM_CITIES.reduce((s,c)=>s+c.viirs,0)/MM_CITIES.length) * yearFactor * (selectedYear===2020?0.92:1);
  const avgTemp  = (MM_CITIES.reduce((s,c)=>s+(landTempMode==="Day"?c.landTempDay:c.landTempNight),0)/MM_CITIES.length)
                   + yearIdx*0.05 + (selectedMonth===0?0:[0.2,0.4,0.6,0.7,0.5,0.3,0.1,-0.1,-0.2,0.0,0.1,0.2][selectedMonth-1]);
  const avgPrecip= (MM_CITIES.reduce((s,c)=>s+c.precip,0)/MM_CITIES.length)
                   * (selectedMonth===0?1:[0.4,0.3,0.3,0.5,0.8,1.4,1.7,1.6,1.5,1.1,0.8,0.5][selectedMonth-1]);
  ...
  function cityVal(city:MMCity):string {
    ...
    if(activeEnvFilter==="ndvi") return (city.ndvi*(monthFactor*(1-yearIdx*0.003))).toFixed(3);
    if(activeEnvFilter==="viirs") return `${(city.viirs*yearFactor*(selectedYear===2020?0.92:1)).toFixed(1)} nW`;
    if(activeEnvFilter==="landtemp"){
      const base=landTempMode==="Day"?city.landTempDay:city.landTempNight;
      const adj=base+yearIdx*0.05+(selectedMonth===0?0:[0.2,0.4,0.6,0.7,0.5,0.3,0.1,-0.1,-0.2,0.0,0.1,0.2][selectedMonth-1]);
      return `${adj.toFixed(1)}°C`;
  ...
  function cityBarColor(city: MMCity): string {
    if(activeEnvFilter==="landcover") return LAND_COLORS[city.dominantLandCover] ?? "#6b7280";
    return activeEnvFilter==="ndvi"?"#7cb342":activeEnvFilter==="viirs"?"#fbbf24":activeEnvFilter==="landtemp"?"#f97316":"#38bdf8";
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className={`${card} p-4`}>
        <p className="text-xs uppercase tracking-widest mb-1" style={{color:lightMode?"#6b7280":"#9ca3af",fontWeight:600}}>
          Observation Count Per Category
        </p>
      ...
      {activeEnvFilter && (
        <div className={`${card} p-4`}>
          ...
          {/* Average */}
          {activeEnvFilter !== "landcover" && (
            <div className={`rounded-lg p-3 mb-3 ${lightMode?"bg-gray-50 border border-gray-200":"bg-[#0f172a] border border-[#2a2f42]"}`}>
              <p className={`text-xs ${subText} mb-1`}>
                {activeEnvFilter==="ndvi"?"MM Average NDVI":activeEnvFilter==="viirs"?"MM Average VIIRS":activeEnvFilter==="landtemp"?`MM Avg Temp (${landTempMode})`:"MM Avg Precipitation"}
              </p>
              ...
              <div className={`h-1.5 rounded-full mt-2 ${lightMode?"bg-gray-200":"bg-[#2a2f42]"}`}>
                <div className="h-1.5 rounded-full" style={{
                  width:`${Math.max(4,Math.min(100,Math.round(
                    activeEnvFilter==="ndvi"?(avgNdvi/0.6)*100:
                    activeEnvFilter==="viirs"?((avgViirs-20)/40)*100:
                    activeEnvFilter==="landtemp"?((avgTemp-(landTempMode==="Day"?33:24))/(landTempMode==="Day"?8:6))*100:
                    ((avgPrecip-50)/250)*100
                  )))}%`,
                  background:activeEnvFilter==="ndvi"?"#7cb342":activeEnvFilter==="viirs"?"#fbbf24":activeEnvFilter==="landtemp"?"#f97316":"#38bdf8",
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {visibleCities.map((city)=>{
                const barColor = cityBarColor(city);
                let barPct = 50;
                if (activeEnvFilter !== "landcover") {
                  const val=activeEnvFilter==="ndvi"?city.ndvi:activeEnvFilter==="viirs"?city.viirs:activeEnvFilter==="landtemp"?(landTempMode==="Day"?city.landTempDay:city.landTempNight):city.precip;
                  const maxVal=activeEnvFilter==="ndvi"?0.45:activeEnvFilter==="viirs"?58:activeEnvFilter==="landtemp"?(landTempMode==="Day"?41:30):250;
                  const minVal=activeEnvFilter==="ndvi"?0.07:activeEnvFilter==="viirs"?27:activeEnvFilter==="landtemp"?(landTempMode==="Day"?34:24):155;
                  barPct=Math.max(6,Math.min(100,((val-minVal)/(maxVal-minVal))*100));
                } else {
                  barPct = city.landCoverBreakdown[city.dominantLandCover] ?? 50;
                }
                return (
                  <div key={city.id}>
                    <div className="flex items-center justify-between mb-0.5 gap-1">
                      <span className={`text-xs ${subText} truncate shrink-0`} style={{minWidth:"60px"}}>{city.name}</span>
                      <span className="text-xs font-bold shrink-0 truncate text-right" style={{color:barColor, maxWidth:"130px", fontSize:"10px"}}>{cityVal(city)}</span>
                    </div>
                    <div className={`h-1 rounded-full ${lightMode?"bg-gray-200":"bg-[#2a2f42]"}`}>
                      <div className="h-1 rounded-full" style={{width:`${barPct}%`,background:barColor,opacity:0.7,transition:"width 0.3s ease"}}/>
                    </div>
                  </div>
                );
              })}
            ...
            {MM_CITIES.length > 5 && (
              <button
                ...
                className={`mt-2.5 w-full flex items-center justify-center gap-1 py-1.5 rounded-md text-xs font-semibold transition-colors ${lightMode?"bg-gray-100 text-gray-600 hover:bg-gray-200":"bg-[#2a2f42]/50 text-gray-400 hover:bg-[#2a2f42]"}`}>
                {showAllCities
...
export function Dashboard() {
  ...
  function toggleLandType(lt: LandType) {
    setSelectedLandTypes(prev => { const n=new Set(prev); n.has(lt)?n.delete(lt):n.add(lt); return n; });
  }
  ...
  // ── Recent Updates: compare to prev month (specific month) or prev year (All) ──
  const recentUpdates = (() => {
    if (selectedMonth === 0) {
      // Compare to previous year
      const prevYear = Math.max(selectedYear - 1, 2014);
      const curTotals = OBS_SITES.reduce((acc,site)=>{
        const c=site.speciesByYear[selectedYear]?.[0]??site.speciesByYear[2024][0];
        return {r:acc.r+c.resident,m:acc.m+c.migratory,lt:acc.lt+c.lightTolerant,ls:acc.ls+c.lightSensitive};
      },{r:0,m:0,lt:0,ls:0});
      const prevTotals = OBS_SITES.reduce((acc,site)=>{
        const c=site.speciesByYear[prevYear]?.[0]??site.speciesByYear[2014][0];
        return {r:acc.r+c.resident,m:acc.m+c.migratory,lt:acc.lt+c.lightTolerant,ls:acc.ls+c.lightSensitive};
      },{r:0,m:0,lt:0,ls:0});
      const curTotal=curTotals.r+curTotals.m+curTotals.lt+curTotals.ls;
      const prevTotal=prevTotals.r+prevTotals.m+prevTotals.lt+prevTotals.ls;
      const diff=curTotal-prevTotal;
      const pct=prevTotal>0?Math.abs(((diff)/prevTotal)*100).toFixed(1):"0";
      const curViirs=MM_CITIES.reduce((s,c)=>s+c.viirs,0)/MM_CITIES.length*(1+(selectedYear-2014)*0.008)*(selectedYear===2020?0.92:1);
      const prevViirs=MM_CITIES.reduce((s,c)=>s+c.viirs,0)/MM_CITIES.length*(1+(prevYear-2014)*0.008)*(prevYear===2020?0.92:1);
      const viirsDiff=curViirs-prevViirs;
      return [
        {
          icon: diff>=0?<TrendingUp size={12} className="text-green-400"/>:<TrendingDown size={12} className="text-red-400"/>,
          bg: diff>=0?"bg-green-500/10":"bg-red-500/10",
          title:`Bird richness ${diff>=0?"increased":"decreased"} by ${pct}% vs ${prevYear}`,
          time:`${selectedYear} vs ${prevYear} (annual)`,
        },
        {
          icon: viirsDiff<=0?<CheckCircle size={12} className="text-green-400"/>:<AlertTriangle size={12} className="text-yellow-400"/>,
          bg: viirsDiff<=0?"bg-green-500/10":"bg-yellow-500/10",
          title:`Avg VIIRS ${viirsDiff<=0?"decreased":"increased"} by ${Math.abs(viirsDiff).toFixed(1)} nW vs ${prevYear}`,
          time:`${selectedYear} vs ${prevYear} (annual)`,
        },
        {
          icon:<Info size={12} className="text-blue-400"/>,
          bg:"bg-blue-500/10",
          title:`Monitoring period: ${prevYear}–${selectedYear} comparison active`,
          time:"Annual summary",
        },
      ];
    } else {
      ...
      const prevMonthYear = selectedMonth === 1 ? Math.max(selectedYear - 1, 2014) : selectedYear;
      const curTotals = OBS_SITES.reduce((acc,site)=>{
        const c=site.speciesByYear[selectedYear]?.[selectedMonth]??site.speciesByYear[2024][0];
        return {r:acc.r+c.resident,m:acc.m+c.migratory,lt:acc.lt+c.lightTolerant,ls:acc.ls+c.lightSensitive};
      },{r:0,m:0,lt:0,ls:0});
      const prevTotals = OBS_SITES.reduce((acc,site)=>{
        const c=site.speciesByYear[prevMonthYear]?.[prevMonth]??site.speciesByYear[2024][0];
        return {r:acc.r+c.resident,m:acc.m+c.migratory,lt:acc.lt+c.lightTolerant,ls:acc.ls+c.lightSensitive};
      },{r:0,m:0,lt:0,ls:0});
      const curTotal=curTotals.r+curTotals.m+curTotals.lt+curTotals.ls;
      const prevTotal=prevTotals.r+prevTotals.m+prevTotals.lt+prevTotals.ls;
      const diff=curTotal-prevTotal;
      const pct=prevTotal>0?Math.abs(((diff)/prevTotal)*100).toFixed(1):"0";
      const prevMoLabel = MONTHS_S[prevMonth-1];
      const curMoLabel = MONTHS_S[selectedMonth-1];
      return [
        {
          icon: diff>=0?<TrendingUp size={12} className="text-green-400"/>:<TrendingDown size={12} className="text-red-400"/>,
          bg: diff>=0?"bg-green-500/10":"bg-red-500/10",
          title:`Species count ${diff>=0?"up":"down"} ${pct}% vs ${prevMoLabel}`,
          time:`${curMoLabel} vs ${prevMoLabel} ${selectedYear}`,
        },
        {
          icon: selectedMonth>=6&&selectedMonth<=9?<AlertTriangle size={12} className="text-yellow-400"/>:<CheckCircle size={12} className="text-green-400"/>,
          bg: selectedMonth>=6&&selectedMonth<=9?"bg-yellow-500/10":"bg-green-500/10",
          title: selectedMonth>=6&&selectedMonth<=9?`Peak wet season — migratory activity elevated`:`Lower light intensity expected for ${curMoLabel}`,
          time:`${curMoLabel} ${selectedYear} seasonal note`,
        },
        {
          icon:<Info size={12} className="text-blue-400"/>,
          bg:"bg-blue-500/10",
          title:`Monitoring update: ${curMoLabel} ${selectedYear} data synced`,
          time:"Monthly update",
        },
      ];
    }
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-5 gap-0 min-h-0 overflow-hidden">
        ...
        <div className="lg:col-span-3 p-4 flex flex-col min-h-[500px] lg:min-h-0 gap-3">
          ...
          {mapView==="historical" && (
            <div className={`flex items-center gap-1.5 flex-wrap px-3 py-2 rounded-lg border ${lightMode?"bg-gray-50 border-gray-200":"bg-[#141c2e] border-[#2a2f42]"}`}>
              ...
              {/* Land Cover — checkbox dropdown */}
              <div ref={lcDropRef} className="relative flex items-center">
                <button
                  onClick={()=>{ setActiveEnvFilter("landcover"); setShowLCDropdown(v=>!v); }}
                  ...
                  style={activeEnvFilter==="landcover"?{background:"#e5393530",borderColor:"#e5393560",border:"1px solid"}:{}}>
                  <span className="w-1.5 h-1.5 rounded-full" style={{background:activeEnvFilter==="landcover"?"#e53935":"#6b7280"}}/>
                  Land Cover
                  {activeEnvFilter==="landcover" && selectedLandTypes.size < LAND_LEGEND.length && (
                    <span className="ml-0.5 px-1 rounded-full text-white font-bold" style={{fontSize:"9px",background:"#e53935",lineHeight:"14px"}}>{selectedLandTypes.size}</span>
                  )}
                ...
                {showLCDropdown && (
                  <div className="absolute top-8 left-0 z-50 bg-[#0f172a] border border-[#2a2f42] rounded-lg shadow-2xl overflow-hidden" style={{minWidth:"190px"}}>
                    <div className="px-3 py-2 border-b border-[#1e2535] flex items-center gap-2 cursor-pointer select-none hover:bg-white/5 transition-colors" onClick={toggleAllLC}>
                      <span className="w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0"
                        style={{borderColor:allLCChecked||someLCChecked?"#e53935":"#4b5563",background:allLCChecked?"#e53935":someLCChecked?"rgba(229,57,53,0.25)":"transparent"}}>
                        {allLCChecked && <svg width="8" height="8" viewBox="0 0 8 8" fill="none" stroke="white" strokeWidth="1.5"><path d="M1 4l2 2 4-4"/></svg>}
                        {someLCChecked && <span style={{width:"6px",height:"2px",background:"#e53935",display:"block",borderRadius:"1px"}}/>}
                      </span>
                    ...
                    <div className="py-1 max-h-52 overflow-y-auto">
                      {LAND_LEGEND.map(lt=>{
                        ...
                        return (
                          <div key={lt} onClick={()=>toggleLandType(lt)}
                            className="flex items-center gap-2 px-3 py-1.5 cursor-pointer select-none hover:bg-white/5 transition-colors">
                            <span className="w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition-all"
                              style={{borderColor:checked?"#e53935":"#4b5563",background:checked?"#e53935":"transparent"}}>
                              {checked && <svg width="8" height="8" viewBox="0 0 8 8" fill="none" stroke="white" strokeWidth="1.5"><path d="M1 4l2 2 4-4"/></svg>}
                            </span>
                            <span className="w-2.5 h-2.5 rounded-sm shrink-0" style={{background:LAND_COLORS[lt],opacity:checked?1:0.35}}/>
              ...
              {/* NDVI, VIIRS, Precip */}
              {["ndvi","viirs","precip"].map(key=>{
                const info={ndvi:{label:"NDVI",color:"#7cb342"},viirs:{label:"VIIRS",color:"#fbbf24"},precip:{label:"Precip",color:"#38bdf8"}}[key as "ndvi"|"viirs"|"precip"]!;
                const isActive=activeEnvFilter===key;
                return (
                  <button key={key} onClick={()=>toggleEnv(key as EnvFilter)}
                    ...
                    <span className="w-1.5 h-1.5 rounded-full" style={{background:isActive?info.color:"#6b7280"}}/>
                    {info.label}
                  </button>
                );
              })}
              ...
              <div ref={ltDropRef} className="relative flex items-center">
                <button
                  onClick={()=>{ toggleEnv("landtemp"); setShowLTDropdown(v=>!v); }}
                    ...
                    <div className="py-1">
                      {(["Day","Night"] as LandTempMode[]).map(m=>(
                        <button key={m} onClick={()=>{ setLandTempMode(m); setActiveEnvFilter("landtemp"); setShowLTDropdown(false); }}
                          ...
                          <span className="text-base">{m==="Day"?"☀️":"🌙"}</span>
                          {m}
          ...
          <div className="flex-1 min-h-0">
            {mapView==="risk"
              ...
              : <MetroManilaMap
                  ...
                  onSiteSelect={setSelectedSiteIdx}
        ...
        <div className="lg:col-span-2 flex flex-col gap-3 p-4 overflow-y-auto">
          ...
          {/* Risk Zone stat cards — only for risk view */}
          {mapView==="risk" && (
            <>
              ...
              {/* Bird Richness Trend — only in risk zone view */}
          ...
          {/* Historical panel — site detail or env panel */}
          {mapView==="historical" && (
            selectedSiteIdx !== null ? (
              <SiteDetailPanel
                site={OBS_SITES[selectedSiteIdx]}
                selectedYear={selectedYear}
                selectedMonth={selectedMonth}
                activeEnvFilter={activeEnvFilter}
                landTempMode={landTempMode}
                lightMode={lightMode}
                card={card}
                subText={subText}
          ...
          <div className={`${card} p-4`}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${lightMode?"bg-gray-100 text-gray-500":"bg-[#2a2f42] text-gray-400"}`}>
                {selectedMonth === 0
                  ? `${selectedYear} vs ${Math.max(selectedYear-1,2014)}`
                  : `${MONTHS_S[selectedMonth-1]} vs ${selectedMonth===1?MONTHS_S[11]:MONTHS_S[selectedMonth-2]}`}
              </span>
            </div>
            <div className="space-y-2">
